...
#Plot the data and model
    print(f"Len xdata {len(xdata)} len VS {len(VS)}")
    fig, axs = plt.subplots(1, 2)
    fig.suptitle('Iv / Vf')
    axs[0].semilogy(xdata, ydata, 'r*', VS, ID, 'b-')
    axs[0].set_title('If/Vf SemiLogaritmic')
    axs[1].scatter(xdata, ydata)
    axs[1].scatter(VS, ID)
    axs[1].set_title('If/Vf Linear')
    fig.supylabel('Current (mA)')
    # axs[0].xlabel('Voltage / V')
    # axs[0].title('Diode I-V Characteristic')
    plt.show()
